...
// Инициализация сцены
  ...
  const renderToolbar = () => {
    return (
      <div className="flex flex-wrap gap-2 mb-4 p-2 bg-gray-800 rounded">
  ...
  const renderAssetPanel = () => {
    ...
    return (
      <div className="w-full md:w-64 h-64 md:h-auto overflow-y-auto bg-gray-800 p-2 rounded">
        <h3 className="text-lg font-bold mb-2">Ассеты</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-1 gap-2">
          {assetList.map(asset => (
            ...
            >
              <div className="w-6 h-6 mr-2 flex-shrink-0 bg-blue-800 rounded flex items-center justify-center">
  ...
  const renderSaveLoadPanel = () => {
    return (
      <div className="w-full md:w-64 bg-gray-800 p-2 rounded">
        <h3 className="text-lg font-bold mb-2">Сохранение и загрузка</h3>
        
        <div className="mb-4">
          <div className="flex mb-2">
            <input
              type="text"
              className="flex-1 px-2 py-1 bg-gray-700 rounded-l"
          ...
          <div className="flex gap-2">
            <button
              className="flex-1 px-3 py-1 bg-blue-600 rounded"
            ...
            <label className="flex-1">
              ...
              />
              <span className="block px-3 py-1 bg-blue-600 rounded text-center cursor-pointer">
                Импорт
              </span>
            </label>
          </div>
        ...
        <h4 className="font-bold mb-1">Сохраненные уровни:</h4>
        {savedLevels.length === 0 ? (
          <p className="text-gray-400 text-sm">Нет сохраненных уровней</p>
        ) : (
          <div className="max-h-40 overflow-y-auto">
            {savedLevels.map(level => (
              <div key={level.name} className="flex items-center mb-1">
                <button
                  className="flex-1 text-left px-2 py-1 bg-gray-700 rounded-l hover:bg-gray-600"
                ...
                <button
                  className="px-2 py-1 bg-red-600 rounded-r hover:bg-red-700"
  ...
  // Функция для рендеринга инструкций в режиме размещения
  const renderPlacementInstructions = () => {
    if (editMode !== 'place' || !selectedAsset) return null;
    
    return (
      <div className="fixed bottom-4 left-4 right-4 bg-black bg-opacity-70 text-white p-3 rounded text-center">
        <p className="text-sm">
          <strong>Режим размещения:</strong> Наведите курсор на объект или сетку, чтобы увидеть, где будет размещен новый объект. Кликните, чтобы подтвердить размещение.
        </p>
      </div>
    );
  };
